...
CHAR_VOCAB_SIZE = 26 # Each char in the word can either be a digit 0-9 or a letter a-z giving a total of 36 pssible characters.
WORD_SIZE =5
...
def char_to_vec_pos(char):
    ...
    if ascii_val >= 97 and ascii_val <=122:
        return (ascii_val-97)                    #return (ascii_val-97)+10
...
def vec_to_word(vector):
    ...
    for idx in char_indices:
        ...
        #elseif vocab_idx <= 35: # a-z
         #   char_code =  (vocab_idx - 10) + ord('a')
        if vocab_idx <= 25:
            char_code = (vocab_idx) + ord('a')
        else:
...
def compare_predictions(predictions,labels):
    ...
    for i,prediction in enumerate(predictions):
        ...
        # print (label, true_word, predicted_word)
